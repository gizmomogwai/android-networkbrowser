...
public abstract class DNSRecord extends DNSEntry {
  ...
  public static class Address extends DNSRecord {
    ...
    boolean handleQuery(JmDNSImpl dns, long expirationTime) {
      ...
      if (dnsAddress != null) {
        if (dnsAddress.sameType(this) && dnsAddress.sameName(this) && (!dnsAddress.sameValue(this))) {
          ...
          Log.d(TAG, "handleQuery() Conflicting probe detected. dns state " + dns.getState() + " lex compare " + lexCompare(dnsAddress));
    ...
    boolean handleResponse(JmDNSImpl dns) {
      DNSRecord.Address dnsAddress = dns.getLocalHost().getDNSAddressRecord(this);
      if (dnsAddress != null) {
        boolean sameType = dnsAddress.sameType(this);
        boolean sameName = dnsAddress.sameName(this);
        boolean sameValue = dnsAddress.sameValue(this);
        if (sameType && sameName && (!sameValue)) {
  ...
  public static class Text extends DNSRecord {
    ...
    boolean handleResponse(JmDNSImpl dns) {
       ...
       * if (info != null) { if (! Arrays.equals(text,info.text)) {
       * info.revertState(); return true; } }
       */
      return false;
    }
